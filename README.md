...
* **Singular Value Decomposition (SVD):** Used to decompose the matrix ( A ) as ( A = U \Sigma V^T ) to uncover the hidden features of users and movies.
...
[
RMSE = \sqrt{ \frac{1}{|R|} \sum_{(i,x) \in R} (\hat{r}*{xi} - r*{xi})^2 }
]
...
* **File Naming Convention:** Name your final submission file as:
  `LA-Project-StudentNumber-FullName`
